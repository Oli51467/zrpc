package com.sdu.sparrow.framework.core.listener;
...
@Component
@Slf4j
public class EnableSparrowListener implements ApplicationListener<ApplicationReadyEvent> {

    @SneakyThrows
    @Override
    public void onApplicationEvent(ApplicationReadyEvent event) {
        ApplicationContext applicationContext = event.getApplicationContext();
        // 获取所有带有 @EnableSparrow 注解的 bean
        Map<String, Object> beansWithAnnotation = applicationContext.getBeansWithAnnotation(EnableSparrow.class);
        for (Object bean : beansWithAnnotation.values()) {
            ...
            // 获取类上的 @EnableSparrow 注解
            EnableSparrow annotation = clazz.getAnnotation(EnableSparrow.class);
            if (null == annotation) {
                continue;
            }
            ...
            // 只处理第一个带有 @EnableSparrow 注解的 bean
            break;
        }
    }
}
