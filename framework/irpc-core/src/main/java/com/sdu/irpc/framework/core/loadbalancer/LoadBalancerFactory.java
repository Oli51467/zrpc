...
public class LoadBalancerFactory {
    ...
    private static final ConcurrentHashMap<String, ObjectWrapper<LoadBalancer>> LOAD_BALANCER_CACHE = new ConcurrentHashMap<>(8);
    ...
    static {
        ...
        LOAD_BALANCER_CACHE.put(LoadBalancerType.ROUND_ROBIN.name(), roundRobin);
        LOAD_BALANCER_CACHE.put(LoadBalancerType.CONSISTENT_HASH.name(), consistentHash);
        LOAD_BALANCER_CACHE.put(LoadBalancerType.RESPONSE_TIME.name(), responseTime);
    }
     ...
     */
    public static ObjectWrapper<LoadBalancer> getLoadbalancer(String loadbalancerType) {
        ...
        if (null == loadBalancerObjectWrapper) {
            ...
            return LOAD_BALANCER_CACHE.get(LoadBalancerType.ROUND_ROBIN.name());
        }
